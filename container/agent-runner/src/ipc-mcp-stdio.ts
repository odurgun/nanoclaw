    ...
    containerConfig: z
      .object({
        additionalMounts: z
          .array(
            z.object({
              hostPath: z.string().describe('Absolute host path to mount'),
              containerPath: z
                .string()
                .describe(
                  'Relative path, mounted at /workspace/extra/{containerPath}',
                ),
              readonly: z.boolean().describe('Whether the mount is read-only'),
            }),
          )
          .optional()
          .describe('Extra host directories to bind-mount into the container'),
      })
      .optional()
      .describe('Container configuration for this group'),
  },
  async (args) => {
    if (!isMain) {
      return {
    ...
    if (args.containerConfig) {
      data.containerConfig = args.containerConfig;
    }
